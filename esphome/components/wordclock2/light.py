...
WordClockRGBLightOutput = neopixelbus_ns.class_(
    "WordClockRGBLightOutput", WordClockLightOutputBase
)
WordClockRBGWLightOutput = neopixelbus_ns.class_(
    "WordClockRBGWLightOutput", WordClockLightOutputBase
)
...
CONFIG_SCHEMA = cv.All(
    ...
    light.ADDRESSABLE_LIGHT_SCHEMA.extend(
        {
            cv.GenerateID(CONF_OUTPUT_ID): cv.declare_id(WordClockLightOutputBase),
...
async def to_code(config):
    ...
    if has_white:
        out_type = WordClockRBGWLightOutput.template(method_template)
    else:
        out_type = WordClockRGBLightOutput.template(method_template)
    rhs = out_type.new()
